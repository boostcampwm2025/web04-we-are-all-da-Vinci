...
export const relativeSimilarity = (a: number, b: number): number => {
...
// 여러 점들 간 총 경로 길이 계산
export const pathLength = (xArr: number[], yArr: number[]): number => {
  let length = 0;

  for (let i = 1; i < xArr.length; i++) {
    length += euclideanDistance(xArr[i - 1], yArr[i - 1], xArr[i], yArr[i]);
  }
