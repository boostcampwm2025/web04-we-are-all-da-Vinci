...
import { createRoomWithPlayers, cleanupContexts } from './fixtures';

test.describe('게임 핵심 플로우', () => {
  test.setTimeout(120000);

  test('방 생성 → 게스트 입장 → 게임 시작 → 1라운드 완주', async ({
    browser,
  }) => {
    const ctx = await createRoomWithPlayers(browser);

    try {
      await test.step('대기실에서 호스트/게스트 모두 게임 시작 버튼이 보인다', async () => {
        await expect(ctx.hostPage.getByText('게임 시작')).toBeVisible();
        await expect(ctx.guestPage.getByText('게임 시작')).toBeVisible();
      });

      await test.step('호스트만 게임 시작 버튼이 활성화된다', async () => {
        await expect(
          ctx.hostPage.getByRole('button', { name: '게임 시작' })
        ).toBeEnabled();
        await expect(
          ctx.guestPage.getByRole('button', { name: '게임 시작' })
        ).toBeDisabled();
      });

      await test.step('게임 시작 후 PROMPT 또는 DRAWING 화면으로 전환된다', async () => {
      ...
      await test.step('라운드 종료 후 결과 화면이 표시된다', async () => {
        await expect(
          ctx.hostPage.getByText(/순위|결과|게임/)
