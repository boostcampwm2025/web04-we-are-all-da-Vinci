  ...
  test('게스트 이탈 → 게임 계속 진행', async ({ browser }) => {
    ...
    try {
      ...
      await test.step('모든 플레이어에게 DRAWING 화면이 표시된다', async () => {
        ...
        await expect(guest1Page.locator('canvas')).toBeVisible({ timeout: 30000 });
        await expect(guest2Page.locator('canvas')).toBeVisible({ timeout: 30000 });
      });
      ...
      await test.step('남은 플레이어들은 게임을 계속 진행한다', async () => {
        ...
        await expect(guest2Page.locator('canvas')).toBeVisible({ timeout: 10000 });
      });
    } finally {
  ...
  test('호스트 이탈 → 호스트 위임', async ({ browser }) => {
    ...
    try {
      ...
      await test.step('게스트에게 게임이 계속 진행된다 (호스트 위임)', async () => {
        await expect(ctx.guestPage.locator('canvas').or(ctx.guestPage.getByText(/라운드|결과|순위/))).toBeVisible({
          timeout: 30000,
        });
      });
    } finally {
